...
class FeedTest(TestCase):
    ...
    def remove_product_categories2_test(self):
        cat = Category.objects.get(pk=7)
        cat2 = Category.objects.get(pk=8)
        f = Feed.objects.get(pk=18)
        p = Product.objects.get(pk=12)
        i = ProductImage.objects.get(pk=4)
        p.product_images.add(i)
        ...
        self.assertEqual(len(f.tiles.first().categories.all()), 1)
        self.assertEqual(f.tiles.first().categories.first(), cat)
        f.remove(p, category=cat2, deepdelete=False)
        self.assertEqual(len(f.tiles.all()), 1)
        self.assertEqual(f.tiles.first().product, p)
        self.assertEqual(f.tiles.first().priority, 35)
        self.assertEqual(len(f.tiles.first().categories.all()), 1)
        self.assertEqual(f.tiles.first().categories.first(), cat)
        f.remove(p, category=cat, deepdelete=False)
        self.assertEqual(len(f.tiles.all()), 0)
        self.assertEqual(f.tiles.first(), None)

    def remove_product_categories3_test(self):
        cat = Category.objects.get(pk=7)
        f = Feed.objects.get(pk=18)
        p = Product.objects.get(pk=12)
        i = ProductImage.objects.get(pk=4)
        p.product_images.add(i)
        f._add_product(p, priority=35, force_create_tile=False)
        self.assertEqual(len(f.tiles.all()), 1)
        self.assertEqual(f.tiles.first().product, p)
        self.assertEqual(f.tiles.first().priority, 35)
        self.assertEqual(len(f.tiles.first().categories.all()), 0)
        self.assertEqual(f.tiles.first().categories.first(), None)
        f.remove(p, category=cat, deepdelete=False)
        self.assertEqual(len(f.tiles.all()), 1)
        self.assertEqual(f.tiles.first().product, p)
        self.assertEqual(f.tiles.first().priority, 35)
    ...
    def remove_content_categories_test(self):
        cat = Category.objects.get(pk=7)
        f = Feed.objects.get(pk=18)
        c = Content.objects.get(pk=6)
        f._add_content(c, priority=35, category=cat, force_create_tile=False)
        ...
        self.assertEqual(len(f.tiles.first().categories.all()), 1)
        self.assertEqual(f.tiles.first().categories.first(), cat)
        f.remove(c, category=cat, deepdelete=False)
        self.assertEqual(len(f.tiles.all()), 0)
        self.assertEqual(f.tiles.first(), None)

    def remove_content_categories2_test(self):
        cat = Category.objects.get(pk=7)
        cat2 = Category.objects.get(pk=8)
        f = Feed.objects.get(pk=18)
        c = Content.objects.get(pk=6)
        ...
        self.assertEqual(len(f.tiles.first().categories.all()), 1)
        self.assertEqual(f.tiles.first().categories.first(), cat)
        f.remove(c, category=cat2, deepdelete=False)
        self.assertEqual(len(f.tiles.all()), 1)
        self.assertEqual(f.tiles.first().content.first(), c)
        self.assertEqual(f.tiles.first().priority, 35)
        self.assertEqual(len(f.tiles.first().categories.all()), 1)
        self.assertEqual(f.tiles.first().categories.first(), cat)
        f.remove(c, deepdelete=False)
        self.assertEqual(len(f.tiles.all()), 0)
        self.assertEqual(f.tiles.first(), None)

    def remove_content_categories3_test(self):
        cat = Category.objects.get(pk=7)
        f = Feed.objects.get(pk=18)
        c = Content.objects.get(pk=6)
        f._add_content(c, priority=35, force_create_tile=False)
        self.assertEqual(len(f.tiles.all()), 1)
        self.assertEqual(f.tiles.first().content.first(), c)
        self.assertEqual(f.tiles.first().priority, 35)
        self.assertEqual(len(f.tiles.first().categories.all()), 0)
