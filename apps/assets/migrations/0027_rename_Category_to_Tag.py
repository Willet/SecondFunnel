...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Rename model 'Category' to 'Tag'
        db.rename_table(u'assets_category', u'assets_tag')
        db.execute("ALTER INDEX assets_category_pkey RENAME TO assets_tag_pkey")
        db.execute("ALTER INDEX assets_category_store_id RENAME TO assets_tag_store_id")

        # Rename M2M table for field products on 'Category'
        db.delete_unique('assets_category_products', ['category_id', 'product_id'])
        db.rename_table(db.shorten_name(u'assets_category_products'), db.shorten_name(u'assets_tag_products'))
        db.rename_column(db.shorten_name(u'assets_tag_products'), 'category_id', 'tag_id')
        db.execute("ALTER INDEX assets_category_products_pkey RENAME TO assets_tag_products_pkey")
        db.execute("ALTER INDEX assets_category_products_category_id RENAME TO assets_tag_products_tag_id")
        db.execute("ALTER INDEX assets_category_products_product_id RENAME TO assets_tag_products_product_id")
        db.create_unique('assets_tag_products', ['tag_id', 'product_id'])

        # Rename model in the contenttypes table
        if not db.dry_run:
            orm['contenttypes.contenttype'].objects.filter(
                app_label='assets', model='category').update(model='tag')

        # Changing field 'Page.feed'
        db.alter_column(u'assets_page', 'feed_id', self.gf('django.db.models.fields.related.ForeignKey')(null=True, to=orm['assets.Feed']))

    def backwards(self, orm):
        # Rename model 'Tag' to 'Category'
        db.rename_table(u'assets_tag', u'assets_category')
        db.execute("ALTER INDEX assets_tag_pkey RENAME TO assets_category_pkey")
        db.execute("ALTER INDEX assets_tag_store_id RENAME TO assets_category_store_id")

        # Rename M2M table for field products on 'Tag'
        db.delete_unique('assets_tag_products', ['tag_id', 'product_id'])
        db.rename_table(db.shorten_name(u'assets_tag_products'), db.shorten_name(u'assets_category_products'))
        db.rename_column(db.shorten_name(u'assets_category_products'), 'tag_id', 'category_id')
        db.execute("ALTER INDEX assets_tag_products_pkey RENAME TO assets_category_products_pkey")
        db.execute("ALTER INDEX assets_tag_products_tag_id RENAME TO assets_category_products_category_id")
        db.execute("ALTER INDEX assets_tag_products_product_id RENAME TO assets_category_products_product_id")
        # Restore unique constraint
        db.create_unique('assets_category_products', ['category_id', 'product_id'])

        # Rename model in the contenttypes table
        if not db.dry_run:
            orm['contenttypes.contenttype'].objects.filter(app_label='assets', model='tag').update(model='category')

        # User chose to not deal with backwards NULL issues for 'Page.feed'
        #raise RuntimeError("Cannot reverse this migration. 'Page.feed' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration
        # Changing field 'Page.feed'
        #db.alter_column(u'assets_page', 'feed_id', self.gf('django.db.models.fields.related.ForeignKey')(null=False, default=orm.Page.objects.get(id=1), to=orm['assets.Feed']))
