...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Copy Tag <-> Product relation over to Category <-> Tiles
        pages = ['aero-gift-guide', 'springisweird', 'mothersday', 'dadsday']

        for page_name in pages:
            try:
                page = orm.Page.objects.get(url_slug=page_name)
            except orm.Page.DoesNotExist:
                print("\t* Can't find <Page '{}'>, skipping".format(page_name))
                continue

            print("\t* Creating categories for <Page '{}'> of <Store '{}'>".format(page.name, page.store.name))

            # Get tags for page
            products = set()
            for t in page.feed.tiles.all():
                products.update(set(t.products.all()))

            tags = set(orm.Tag.objects.filter(products__in=products))
            
            # Get or create categories from tags & transfer relations over
            for tag in tags:
                # Get or create category
                try:
                    cat = orm.Category.objects.get(name=tag.name, store=tag.store)
                except orm.Category.DoesNotExist:
                    cat = orm.Category(name=tag.name, store=tag.store)
                    cat.save()
                    print("\t\tCreated <Category '{}'>".format(tag.name))
                
                # Add tiles with tagged products to category
                tiles = set(page.feed.tiles.filter(products__in=tag.products.all()))
                cat.tiles.add(*list(tiles))

    def backwards(self, orm):
        "Write your backwards methods here."
        # Remove all categories
        print("\t* Deleting all <Category> objects")
