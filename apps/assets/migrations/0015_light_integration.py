...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        landing_regex = re.compile(r"apps/pinpoint/static/pinpoint/themes/([a-zA-Z_-]+)/index.html",re.IGNORECASE)
        ad_regex = re.compile(r"apps/ads/static/ads/themes/([a-zA-Z_-]+)/index.html", re.IGNORECASE)
        for theme in orm.Theme.objects.all():
            # change template of themes (LANDING PAGE)
            # from `apps/pinpoint/static/pinpoint/themes/madewell/index.html`
            # to   `madewell/landingpage/default/index.html`
            results = landing_regex.match(theme.template)
            if results:
                client_name = results.groups()[0]
                theme.template = "%s/landingpage/default/index.html" % client_name

            # change template of themes (AD)
            # from `apps/ads/static/ads/themes/nastygal/index.html`
            # to `nastygal/ad/default/index.html`
            results = ad_regex.match(theme.template)
            if results:
                client_name = results.groups()[0]
                theme.template = "%s/ad/default/index.html" % client_name
