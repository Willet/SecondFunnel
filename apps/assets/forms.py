        ...
        if products:
            for p in products.all():
                if not p.store == store:
                    raise ValidationError({'products': [u"Products in tag must have same store as tag."]})
        ...
        if tiles:
            for t in tiles.all():
                ...
                if not feed.store == store:
                    raise ValidationError({'tiles': [u"Tiles in category must have same store as category."]})
