...
def scrapy(command='crawl', spider='', args=''):
    """
    Runs the `scrapy` scraper on a remote instance determined by the
    environment.

    fab <test|production> scrapy:spider='<spider-name>',args='<arguments passed to scrapy>'

    `Args` is optional, but if provided, will pass arguments along to scrapy.
    Because of how Fabric handles arguments, commas and equal signs need to
    be escaped (e.g. `\=`, `\,`).

    Each spider has a `name` attribute which Scrapy uses to pick a scraper to
     run. If you use a spider name and no spider has that name,
     no spider will be run.

    To persist data to the database, a spider's `store_slug` property must
    match the `store_slug` found in the database, otherwise Scrapy will just
    drop the item.

    Example Usage:
    scrape the `sejour` category (10033) to the database,
    and also to `feed.json`

        fab test scrapy:spider='voyage-prive',args='-a categories\=10033 -o
        feed.json -t json'
    """
    print args
