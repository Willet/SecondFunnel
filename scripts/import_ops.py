...
def execute_scraper(instance_type, store_id):
...
def scraper(instance_type='test', store_id=None):
    print green("Scraping to {0}...".format(instance_type))
    instances = get_instances(instance_type)
    instances_dns = [i.public_dns_name for i in instances]
    execute(execute_scraper, instance_type, store_id, hosts=[instances_dns[0]])
